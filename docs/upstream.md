...
The adaptation keeps the two graph tutorials, the explicit retrieve/grade/draft/rewrite pattern, and the prebuilt ReAct pattern. The two RAG notebooks now share executable modules with the CLI and tests. GoodMem replaces Chroma, local BGE-M3, and client-side splitting. The same six source page paths are used, with official Markdown preferred over full-page HTML scraping.
