...
def setup(settings: Settings, *, initialize: bool = False, with_reranker: bool = False) -> dict:
    if initialize:
        try:
            ...
            save_json(settings.runtime / "credentials.json", {
                "base_url": settings.base_url, "api_key": initialized.root_api_key,
            })
    try:
        previous = settings.state()
    except ValueError:
        previous = {}
    state = {"base_url": settings.base_url, "namespace": settings.namespace,
             "spaces": {}, "documents": [], "chunking": CHUNKING}
    identity = uuid.uuid5(uuid.NAMESPACE_URL, f"goodmem-rag:{settings.namespace}")
    with settings.client() as client:
        ...
        if embedder_id:
            ...
            def create_embedder():
                if not key:
                    raise ValueError("Set GOODMEM_EMBEDDER_ID or EMBEDDING_API_KEY / COHERE_API_KEY")
                return client.embedders.create(
                    embedder_id=embedder_id, display_name=f"{settings.namespace}: {model}",
                    model_identifier=model, api_key=key,
                )
        ...
        if reranker_id:
            ...
            _ensure(lambda: client.rerankers.get(id=reranker_id), lambda: client.rerankers.create(
                reranker_id=reranker_id, display_name=f"{settings.namespace}: {rerank_model}",
                model_identifier=rerank_model,
                api_key=os.environ["COHERE_API_KEY"],
            ))
        state["reranker_id"] = reranker_id
        with httpx.Client(timeout=60, follow_redirects=True,
                          headers={"User-Agent": "Agentic-RAG-GoodMem/0.1"}) as http:
            for collection, urls in SOURCES.items():
                ...
                _ensure(lambda: client.spaces.get(id=space_id), lambda: client.spaces.create(
                    space_id=space_id, name=f"{settings.namespace}: {collection}",
                    space_embedders=[{"embedder_id": embedder_id}],
                    default_chunking_config=CHUNKING,
                    labels={"application": "agentic-rag-goodmem", "collection": collection},
                ))
                ...
                for url in urls:
                    ...
                    memory = _ensure(
                        lambda: client.memories.get(id=memory_id),
                        lambda: client.memories.create(
                            memory_id=memory_id, space_id=space_id,
                            original_content=document["text"], content_type="text/plain",
                            original_content_ref=url,
                            metadata={k: v for k, v in document.items() if k != "text"}
                                     | {"application": "agentic-rag-goodmem"},
                        ),
                    )
                    wait_for_memory(client, memory.memory_id, settings.ingest_timeout)
                    current_ids.add(memory_id)
                    state["documents"].append({k: v for k, v in document.items() if k != "text"}
                ...
                for old in client.memories.list(space_id=space_id):
                    if (old.metadata or {}).get("application") == "agentic-rag-goodmem" and (
                        old.memory_id not in current_ids
                    ):
